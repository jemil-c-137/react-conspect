...
Те изменения текста которые мы видим(`view`) в `UI` они не проходят через Логику. `UI` меняет сам себя.
...
`Пользователь ввел букву(Она еще не видна в текстовом поле) => Буква отправилась в state => Из стейка попадает в UI и отображается`
