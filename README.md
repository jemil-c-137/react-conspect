...
Пагинцаия - постраничная выдача иформации. 
Когда мы отправляем запрос на сервер, что бы получить данные, например за списком пользователей, он не отдает сразу всех пользователей, т.к. это бы приводило к большой нагрузке на сервера и он может не выдержать. 
Поэтому он отправляет данные порционно, `page`'ами или страницами.

![image](./imgs/pagina.png)

Что бы получить определенную страницу, в get запросе, нужно указывать номер страницы, например: 
```
get:'someservice/api/users?pageNumber=2'
```
**?pageNumber=2** и есть номер страницы 
наименование и номера этих страниц определяет серверный разработчик, backend. 
В этом примере сервер возращает по 5 пользователей на странице, но что если необходимо получить по 3 пользователя на странице.
для такого существует **pageSize** или **count** и т.п. это должно быть указано в api документации. Это называется get-параметрами.

```
get:'someservice/api/users?pageNumber=2&pageSize=3'
```
теперь данные будут приходить по 3 пользователя на странице. 

Это все может понадобиться для порционного показа данных на веб сайте, пагинации - это когда на сайте есть переключатели с номерами страниц

![image](https://sitechecker.pro/wp-content/uploads/2017/12/prevnext-implementing-of-pagination-on-Ahrefs-blog.png)

# 74 componentDidupdate() 

Один из методов жизненого цикла компонента, позволяет выполнять операции, когда в компоненте меняются стейт или пропсы.


# 75  redux-form Формы

До этого стейт для стейт менеджмента форм нам приходилось "вручну". 
Для усорения процесса можно подключить библиотеку redux-form.
```
npm install redux-from
```
Форму создаем в компоненте, которую потом будет импортировать в разметку.
```JSX
...
```
Что бы "подключить" эту форму к библиотеке нужно ее обернуть компонентой-контакт, эта компонента берет на себя взаимодействие формы и стора, и у нас нет необходимости организовывать стейт менеджмент для каждого инпута, flux-круговорот никто не отменял.

Создание обертки похоже на создание контейнеров для презентационных компоненn(функция connect), тоже черз функцию

Объявляем следующую функцию: 
```javascript
const MessageReduxForm = reduxForm( {form: 'message'})(MessageForm)
```
Функция `reduxForm` импортируется из библиотеки redux-form, первым вызовом принимает объект, где в ключе `form` мы задаем уникальное имя формы. Второй вызов вернет компонент `MessageForm` обернутый компонентом `MessageReduxForm`.

Теперь в разметку можно вставить обертку
...
```
И зарефакторим самну форму.
Что бы все работало корректно необходимо стандартные html `input` заменить компонентом `<Field />`:
```JSX
const MessageForm = (props) => {
  return(
    <form>
...
```
У этого компонента должен быть обязательный атрибут `component` с указание типа компонента (*input*, *checkbox* и т.д.), так же следует указывать имена полей формы, по этим именам можно будет получать value поля.
у тега форм необходимо в атрибуты добавить onSubmit со значением `props.submithandler`, который и будет выполнять нужные нам действия после `sumbit`'а формы. Получает форма этот хэндлер через пропсы от оберточного компонента.

![image](./imgs/reduform.png)

Что бы получить данные из формы и сделать с ними операции необходимо объявить функцию(она и будет `handleSubmit`). 

![image](./imgs/formData.png)

И передать эту функцию через пропсы из обертки в форму.

![image](./imgs/formContainer.png)

Вот так будет выглядеть полная картина. 

![image](./imgs/reduxFormCom.png)

Тестим :)

![image](./imgs/tic.png)![image](./imgs/tac.png)
